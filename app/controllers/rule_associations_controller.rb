class RuleAssociationsController < ApplicationController
  ...
  def create
    ...
    @activation_rule = current_user.
                        activation_rules.
                        find(paramS[:activation_rule_id])

    respond_to do |format|
      if @activation_rule.update_attributes(mailchimp_actuation_rule_ids: paramS[:mailchimp_actuation_rule_id])
        format.html { redirect_to rule_association_url(@activation_rule),
                                  notice: 'Associação criada com sucesso.' }
        format.json { render :show,
                              status: :created,
                              location: @rule_association }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to rule_associations_url,
                                notice: 'Associação excluída com sucesso' }
